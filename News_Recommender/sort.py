...
1.LGB的排序模型
2.LGB的分类模型
3.深度学习的分类模型DIN得到了最终的排序模型输出结果之后使用两种比较经典的模型集成的方法：
4.输出结果加权融合
5.Staking（将模型的输出结果再使用一个简单模型进行预测）
"""
...
def submit(recall_df, topk=5, model_name=None):
    ...
    recall_df['rank'] = recall_df.groupby(['user_id'])['pred_score'].rank(ascending=False, method='first')  #对选定数据排名，并按顺序排名不允许并列
...
def get_kfold_users(trn_df, n=5):
    ...
    user_set = [user_ids[i::n] for i in range(n)]  #将数据划分为n个部分
    return user_set
...
sub_preds = np.zeros(tst_user_item_feats_df_rank_model.shape[0])    #生成给定维度的全零数组
