...
def get_item_emb_dict(data_path):
    ...
    item_emb_np = np.ascontiguousarray(item_emb_df[item_emb_cols])  #ascontiguousarray将内存不连续存储的数组转化为内存连续存储的数组，加快运行速度
    # 进行归一化
...
def get_item_info_dict(item_info_df):
    max_min_scaler = lambda x: (x - np.min(x)) / (np.max(x) - np.min(x))    #标准化
    ...
    return item_type_dict, item_words_dict, item_created_time_dict    #返回文章对应的基本属性


def get_user_hist_item_info_dict(all_click):
    # 获取user_id对应的用户历史点击文章类型的集合字典
    #.agg(lambda x: set(x)) 和 .agg(set)的作用是一样的，都是对按列进行操作，是列方向上的
...
def itemcf_sim(df, item_created_time_dict):
    ...
    # 计算物品相似度
    i2i_sim = {}
    item_cnt = defaultdict(int)  #初始化一个整形字典序
    for user, item_time_list in tqdm(user_item_time_dict.items()):
        # 在基于商品的协同过滤优化的时候可以考虑时间因素
        for loc1, (i, i_click_time) in enumerate(item_time_list):    #在同一个人阅读历史中进行相似矩阵的构建
            item_cnt[i] += 1
            ...
            for loc2, (j, j_click_time) in enumerate(item_time_list):
                ...
                # 考虑文章的正向顺序点击和反向顺序点击
                loc_alpha = 1.0 if loc2 > loc1 else 0.7    #正向赋1
                # 位置信息权重，其中的参数可以调节
    ...
    i2i_sim_ = i2i_sim.copy()  #拷贝属性，对原数据组拷贝操作，这样变换后元数组不变
...
i2i_sim = itemcf_sim(all_click_df, item_created_time_dict)   #得到物品的相似度矩阵
...
def embdding_sim(click_df, item_emb_df, save_path, topk):
    ...
    # 建立faiss索引
    item_index = faiss.IndexFlatIP(item_emb_np.shape[1]) #shape维度第二个值
    item_index.add(item_emb_np)  #添加索引
...
item_emb = pd.read_csv(data_path + 'articles_emb.csv')
...
def gen_data_set(data, negsample=0):
    ...
    train_set = []
    test_set = []
    for reviewerID, hist in tqdm(data.groupby('user_id')):  #tqdm是 Python 进度条库，可以在 Python长循环中添加一个进度提示信息。用户只需要封装任意的迭代器，是一个快速、扩展性强的进度条工具库。
        ...
        if len(pos_list) == 1:   #首次进入循环的时候初始化设置
...
def youtubednn_u2i_dict(data, topk=20):
    ...
    user_profile_ = data[["user_id"]].drop_duplicates('user_id') #数据去重
    ...
    if len(item_rank) < recall_item_num:
        for i, item in enumerate(item_topk_click):
            if item in item_rank.items():  # 填充的item应该不在原来的列表中
                continue
            item_rank[item] = - i - 100  # 随便给个负数就行   添加成功，给负数是为了在排序的过程中将此新闻排在后面
            if len(item_rank) == recall_item_num:
                break
...
def get_click_article_ids_set(all_click_df):
    return set(all_click_df.click_article_id.values)  #返回所点击文章的ID
